...
export async function handler(chatUpdate) {
    ...
    global.img = 'https://telegra.ph/file/1250f09fa201854a06122.jpg' 
    ...
    try {
        ...
        try {
            ...
            if (user) {
                ...
                if (!isNumber(user.ayamgoawaaag)) user.ayamgoawaaag = 0
                ...
                if (!isNumber(user.kentanggoawaaag)) user.kentanggoawaaag = 0
                ...
                if (!isNumber(user.awaaadang)) user.awaaadang = 0
                ...
                if (!isNumber(user.stawaaagth)) user.stawaaagth = 0
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `Limit Kamu Habis, Beli Dengan Cara *${usedPrefix}beli limit*`, m)
...
let vn = "./vn/lusiapa.mp3"
let mssg = {
        rowner: '❗ Perintah ini hanya dapat digunakan Oleh awaaa!',
        owner: '❗ Perintah ini hanya dapat digunakan Oleh awaaa!',
        mods: '❗Perintah ini Hanya dapat digunakan Oleh awaaa!'}[type]
        if (mssg) return conn.sendFile(m.chat, vn, "owner.mp3", null, m, true, {
		type: "audioMessage",
		ptt: true,
	})
let msg = {
        premium: '❗Khusus User Premium Silahkan Hubungi Ownerku Jika Mau Buy Premium Wa.me/6282389924037',
        group: '❗Khusus Di Dalam Grup',       
        botAdmin: '❗Jadikan RanBot Sebagai Admin Terlebih Dahulu Agar Bisa Menggunakan Fitur Ini',
        restrict: '❗Restict Belum Di Nyalakan Untuk Chat Ini'}[type]
  if (msg) return conn.reply(m.chat, msg, fkon)
  let daftar = {
  unreg: '❗Hai Kak, Sebelum Menggunakan Fiturku, Kamu Harus Daftar Ke Database Terlebih Dahulu\nCaranya Ketik .daftar namakamu.umurkamu\n\nContoh : .daftar Yaemiko.20'}[type]
